...
export class LeoView {
    ...
    public get activeTopMenu(): HTMLDivElement | null {
        return this.menuManager?.activeTopMenu ?? null;
    }

    public set activeTopMenu(value: HTMLDivElement | null) {
        if (this.menuManager) {
            this.menuManager.activeTopMenu = value;
        }
    }

    public get focusedMenuItem(): HTMLDivElement | null {
        return this.menuManager?.focusedMenuItem ?? null;
    }

    public set focusedMenuItem(value: HTMLDivElement | null) {
        if (this.menuManager) {
            this.menuManager.focusedMenuItem = value;
        }
    }

    public get topLevelItems(): HTMLDivElement[] {
        return this.menuManager?.topLevelItems ?? [];
    }

    public get topLevelSubmenus(): Map<HTMLDivElement, HTMLElement> {
        return this.menuManager?.topLevelSubmenus ?? new Map();
    }
    ...
    constructor() {
        ...
        this.menuManager = new MenuManager(this.TOP_MENU);
        this.menuManager.setRestoreLastFocusedElementCallback(() => {
            this.restoreLastFocusedElement();
        });
        this.menuManager.setDoCommandCallback((command: string) => {
            workspace.controller.doCommand(command);
        });
    }
    ...
    public buildMenu(entries: MenuEntry[], level = 0): HTMLElement {
        return this.menuManager.buildMenu(entries, level);
    }

    public refreshMenu(entries: MenuEntry[], level = 0): void {
        this.menuManager.refreshMenu(entries, level);
    }

    public openTopMenu(item: HTMLDivElement, sub: HTMLElement | null, level: number): void {
        this.menuManager.openTopMenu(item, sub, level);
    }

    public positionSubmenu(parentItem: HTMLDivElement, submenu: HTMLElement, level: number): void {
        this.menuManager.positionSubmenu(parentItem, submenu, level);
    }

    public closeAllSubmenus(): void {
        this.menuManager.closeAllSubmenus();
    }

    public focusMenuItem(item: HTMLDivElement | null): void {
        this.menuManager.focusMenuItem(item);
    }

    public closeMenusEvent(e: MouseEvent): void {
        this.MENU.style.display = "none";
        const target = e.target as Element;
        this.menuManager.closeMenusEvent(target);
    }
